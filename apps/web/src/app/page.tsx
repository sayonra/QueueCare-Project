...
type CounterSnapshot = Counter & { branch: { id: number; name: string }; current_ticket: Ticket | null; waiting_tickets: Ticket[]; skipped_tickets: Ticket[]; waiting_count: number; refreshed_at: string };
type Dashboard = { waiting_now: number; active_counters: number; served_today: number; skipped_today: number; cancelled_today: number; live_activity: { number: string; status: string; service: string; counter: string | null; updated_at: string }[]; refreshed_at: string };
    ...
    <main className="min-h-screen bg-[#F4F8FF] text-[#0B1736]">
      <div className="mx-auto grid min-h-screen max-w-[1600px] lg:grid-cols-[248px_1fr]">
        <aside className="hidden border-r border-[#DDE7F5] bg-white/80 px-5 py-7 backdrop-blur lg:flex lg:flex-col">
          ...
          <div className="mt-auto rounded-2xl bg-[#0B1736] p-4 text-white"><p className="text-xs font-semibold text-[#72DDB8]">SPRINT 3</p><p className="mt-2 text-sm font-bold">Live counter workflow</p><div className="mt-3 h-1.5 rounded-full bg-white/15"><div className="h-full w-full rounded-full bg-[#72DDB8]" /></div><p className="mt-2 text-xs text-white/60">Polling every 5 seconds</p></div>
        </aside>
...
function DashboardWorkspace({ branch, token }: { branch: Branch; token: string }) {
  ...
  return <div className="mt-7 space-y-6"><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Waiting now" value={dashboard.waiting_now} detail="Customers in queue" color="blue"/><Metric label="Active counters" value={dashboard.active_counters} detail="Open and available" color="mint"/><Metric label="Served today" value={dashboard.served_today} detail="Completed tickets" color="violet"/><Metric label="Exceptions" value={dashboard.skipped_today + dashboard.cancelled_today} detail={`${dashboard.skipped_today} skipped · ${dashboard.cancelled_today} cancelled`} color="amber"/></section><Panel title="Live queue activity" subtitle="Latest ticket changes across this branch."><div className="overflow-x-auto"><table className="w-full min-w-[560px] text-left text-sm"><thead className="text-xs uppercase tracking-wider text-[#7D8EAA]"><tr><th className="pb-3">Ticket</th><th className="pb-3">Service</th><th className="pb-3">Counter</th><th className="pb-3">Status</th></tr></thead><tbody>{dashboard.live_activity.map((item) => <tr key={`${item.number}-${item.updated_at}`} className="border-t border-[#E4ECF7]"><td className="py-4 font-black">{item.number}</td><td>{item.service}</td><td>{item.counter ?? "—"}</td><td><span className="rounded-full bg-[#DCEBFF] px-3 py-1 text-xs font-bold capitalize text-[#0B5CFF]">{item.status}</span></td></tr>)}</tbody></table>{!dashboard.live_activity.length && <p className="py-10 text-center text-[#7D8EAA]">No ticket activity yet today.</p>}</div></Panel><a href={`/display/${branch.id}`} target="_blank" className="inline-flex h-11 items-center rounded-xl bg-[#0B1736] px-5 text-sm font-bold text-white">Open public display ↗</a></div>;
}
